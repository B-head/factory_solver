...
-- analytic centre -- flow split across the tied recipes, and that split can shift
-- under a different presolve formulation or warm start. Perturbing each recipe's
-- epsilon by a tiny recipe-specific amount shrinks that face to a single vertex,
-- so the LP has one canonical optimum independent of formulation.
--
...
-- enough to separate tied optima past the IPM's relative tolerance (~1e-6), small
-- enough that it never competes with the flat epsilon's futile-collapse or, one
-- tier up, with source_cost's material-efficiency tie-break. At 2^-4 the
-- effective epsilon lands in [2^-6, 2^-6 + 2^-10) ~= [0.0156, 0.0166): still
-- below the ~2^-6 safe-window edge, still ~60x below source_cost = 1, and the
-- ~6% relative spread between two recipes clears IPM tolerance. It cannot flip
-- the flat epsilon's activity ranking either -- the gaps there are factors (5 vs
-- 10 lines), which a <= 1 jitter can never invert.
...
function M.create_problem(solution_name, constraints, production_lines)
    ...
    for i, line in ipairs(all_lines) do
        ...
        if bridge then
            ...
            -- stay free relative to source / elastic, or the LP would skip a
            -- needed temperature conversion). But when several bridges can route
            -- the same fluid they are all cost-0 and the LP is indifferent
            -- between them -- a degenerate face whose free dimension lives
            -- entirely on the bridge variables, and the interior-point method
            -- picks its analytic centre cold, so the realized bridge flows drift
            -- across warm-started re-solves (the temperature-plumbing wobble:
            -- the dominant degeneracy on temperature-heavy chains, orders above
            -- the recipe wobble). The recipe jitter above cannot reach it --
            -- bridges take neither the base epsilon nor that jitter. So give each
            -- bridge the SAME deterministic key-hash jitter, WITHOUT the
            -- recipe_epsilon base: a distinct infinitesimal cost in [0, 2^-10)
            -- that picks one canonical routing without pricing the hop. Still
            -- ~three orders below source_cost, so it never tips a real decision
            -- or makes the LP skip a bridge.
            local bridge_cost = slack_cost + recipe_epsilon * jitter_strength * key_unit_hash(objective_name)
            problem:add_objective(objective_name, bridge_cost, false)
        else
            ...
            -- slack_cost = 0. Both carry the flat recipe_epsilon plus a
            -- per-recipe hash jitter (both inside the recipe_epsilon tier) so
            -- genuinely tied recipes resolve to one canonical vertex instead of
            -- an analytic-centre split.
            ...
            if is_source_line(line) and line.products[1] then
                recipe_cost = source_cost_for(tn.typed_name_to_variable_name(line.products[1])) + recipe_cost
            end
