...
function M.build_material_graph(production_lines)
    ...
    for _, line in ipairs(production_lines) do
        ...
        if line.fuel_spent_fluid then
            products[#products + 1] = tn.typed_name_to_variable_name(line.fuel_spent_fluid)
        end
...
function M.compute_net_flow(production_lines, scc_set)
    ...
    for _, line in ipairs(production_lines) do
        ...
        if not touches and line.fuel_spent_fluid
            and scc_set[tn.typed_name_to_variable_name(line.fuel_spent_fluid)] then
            touches = true
        end
        if not touches then
            for _, ing in ipairs(line.ingredients) do
        ...
        if touches then
            ...
            if line.fuel_spent_fluid then
                local v = tn.typed_name_to_variable_name(line.fuel_spent_fluid)
                if scc_set[v] then
                    net[v] = net[v] + line.fuel_spent_fluid.amount_per_second
                end
            end
            for _, ing in ipairs(line.ingredients) do
...
local function build_scc_matrix(production_lines, scc)
    ...
    for _, line in ipairs(production_lines) do
        ...
        if line.fuel_spent_fluid then
            add(tn.typed_name_to_variable_name(line.fuel_spent_fluid),
                line.fuel_spent_fluid.amount_per_second)
        end
        for _, ing in ipairs(line.ingredients) do
