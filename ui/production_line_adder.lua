...
---@param kind string
---@return string[]
local function pickable_recipe_names(reference, recipe_names, kind)
...
function handlers.on_init_choose_visiblity(event)
    ...
    elem.visible = allowed
        and 0 < #pickable_recipe_names(reference_typed_name, recipe_names, kind)
end
...
function handlers.on_make_choose_table(event)
    ...
    recipe_names = pickable_recipe_names(choose_typed_name, recipe_names, kind)
