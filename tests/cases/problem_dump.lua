...
table.insert(cases, {
    name = "dump_normalized_lines round-trips is_source / is_sink flags",
    -- The source/sink class must survive dump -> load so a replayed fixture
    -- prices its <source> recipe at source_cost again (create_problem reads the
    -- flag, not the name). is_bridge is intentionally NOT dumped: bridges are
    -- regenerated by create_temperature_bridges and never part of the line set.
    run = function()
        local lines = {
            {
                recipe_typed_name = { type = "virtual_recipe", name = "<source>item/plate", quality = "normal" },
            ...
            {
                recipe_typed_name = { type = "virtual_recipe", name = "<sink>item/waste", quality = "normal" },
                products = {},
                ingredients = { { type = "item", name = "waste", quality = "normal", amount_per_second = 1 } },
                power_per_second = 0,
                pollution_per_second = 0,
                is_sink = true,
            },
        ...
        local dumped = cp.dump_normalized_lines(lines)
        local loaded = load_dump(dumped)
        harness.assert_true(deep_eq(loaded, lines), "is_source/is_sink survive round-trip")
        harness.assert_true(loaded[1].is_source == true, "source flag loaded")
        harness.assert_true(loaded[2].is_sink == true, "sink flag loaded")
        harness.assert_eq(cp.dump_normalized_lines(loaded), dumped, "re-dump is stable")
    end,
})
