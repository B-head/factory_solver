...
-- material whose |basic_source| cost is scaled away from the item default.
...
table.insert(cases, {
    name = "fuel_ingredient is treated as a consumed material with its own basic_source",
    ...
    -- like a real ingredient: it gets a |basic_source| (no producer here) and
    ...
    run = function()
        ...
        harness.assert_near(vars.x["|basic_source|item/iron-ore/normal"], 10, 0.05, "iron-ore drawn 1:1")
        -- The load-bearing assertion: fuel is consumed and sourced, scaling
        -- with recipe rate (0.05 coal/run * 10 runs = 0.5/s).
        harness.assert_near(vars.x["|basic_source|item/coal/normal"], 0.5, 0.01,
            "coal fuel sourced at recipe_rate * fuel_amount")
    end,
})

table.insert(cases, {
    name = "<heat> basic_source uses the heat-scaled source_cost so a heat-fed chain does not collapse",
    -- create_problem.source_cost_for prices |basic_source| per material kind:
    ...
    run = function()
        ...
        harness.assert_near(vars.x["|basic_source|virtual_material/<heat>/normal"], 1e7, 1e3,
