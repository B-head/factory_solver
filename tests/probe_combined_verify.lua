...
-- Usage:
--   pwsh tests/run_corpus.ps1 -Driver tests/probe_combined_verify.lua -Collect '^seed=' -Out <tsv>
...
local function relax_total(p, x) local s = 0; for k, pr in pairs(p.primals) do if pr.kind == "elastic" then s = s + math.abs(x[k] or 0) end end; return s end
