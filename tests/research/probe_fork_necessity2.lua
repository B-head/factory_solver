...
local per_scc = {}
local kCls, rCls = { scc = 0, fork = 0, other = 0 }, { scc = 0, fork = 0, other = 0 }
local knames = {}
for i, b in ipairs(order2) do
    ...
    if st == "finished" then
        ...
        kCls[cls] = kCls[cls] + 1
        if cls == "other" then knames[#knames + 1] = b:gsub("^%w+/", "") end
        for m in pairs(basenodes[b] or {}) do
            local id = sccid[m]
            if id and sccsize[id] >= 2 then per_scc[id] = (per_scc[id] or 0) + 1; break end
        end
    end
end
local nsccG = 0
for _, sz in pairs(sccsize) do if sz >= 2 then nsccG = nsccG + 1 end end
io.write(string.format(
    "fk2 n_grp=%d n_min=%d kScc=%d kFork=%d kOther=%d rScc=%d rFork=%d rOther=%d nsccG=%d trunc=%d knames=%s seed=%s\n",
    #order2, #keptlist + (#order2 - math.min(#order2, PROBE_CAP)),
    kCls.scc, kCls.fork, kCls.other, rCls.scc, rCls.fork, rCls.other,
    nsccG, (#order2 > PROBE_CAP) and 1 or 0,
    (#knames > 0) and table.concat(knames, ",") or "-", fid))
for id, sz in pairs(sccsize) do
    if sz >= 2 then
        io.write(("fs2 id=%d size=%d kept=%d pf=%d cf=%d xf=%d seed=%s\n")
            :format(id, sz, per_scc[id] or 0, sccPF[id] or 0, sccCF[id] or 0, sccXF[id] or 0, fid))
    end
end
