...
-- Static (solve-free) elastic reduction via graph fixpoints, measured against
-- the Phase-I oracle. The reference's producible/consumable sets are
-- LP-defined (joint-demand solves); this probe builds two purely-static
-- approximations over the subject_terms net-coefficient graph and measures
-- how much the missing ratio/fork coupling actually costs on the corpus:
--   (a) simple reachability -- producible(m): some recipe makes m with all
--       net-inputs producible (free ports seed); consumable is the mirror.
--       Optimistic: ignores co-product drainage / co-input supply.
--   (b) coupled -- running r harder needs ALL net-inputs producible AND ALL
--       net-outputs consumable (mutually recursive least fixpoint).
--       Pessimistic: keeps more, safer.
-- Filter: drop a group's shortage side iff every variant is producible, its
-- surplus side iff every variant is consumable. Evaluation per variant:
-- full Phase-I (safe iff sumArt~0), then restricted L2 vs all-elastic L2.
--   luajit tests/research/probe_static_reduce.lua <dump> [verbose]
...
local groups, gorder = {}, {} -- base -> { short = {keys}, surp = {keys}, vars = {mat=true} }
...
-- ======== fixpoints ========
-- (a) simple: producible needs inputs producible; consumable needs outputs
-- consumable. (b) coupled: a recipe is runnable-harder only when ALL its
-- inputs are producible AND ALL its outputs are consumable; then every output
-- becomes producible and every input consumable. Both are least fixpoints
-- computed by naive iteration (graphs are small: O(recipes * rounds)).
local function fixpoints(coupled)
    local P, C = {}, {}
    ...
    while changed do
        ...
        for _, r in ipairs(recs) do
            ...
            local fire_p = coupled and (in_ok and out_ok) or in_ok
            local fire_c = coupled and (in_ok and out_ok) or out_ok
            if fire_p then
...
if st0 ~= "finished" then io.write(("sf st=base_%s seed=%s\n"):format(tostring(st0), fid)); return end
local S0 = group_stats(base, x0)
local base_tot = S0.imp + S0.dmp

-- ======== evaluate one variant ========
local function evaluate(tag, P, C)
    local trial, kept_any, drop_short, drop_surp = {}, 0, 0, 0
    for _, b in ipairs(gorder) do
        local g = groups[b]
        local prod_all, cons_all = true, true
        ...
        if prod_all then
            ...
            drop_short = drop_short + 1
        end
        if cons_all then
            for _, k in ipairs(g.surp) do trial[#trial + 1] = k end
            drop_surp = drop_surp + 1
        end
        if not (prod_all and cons_all) then kept_any = kept_any + 1 end
    end
    table.sort(trial)
    ...
    io.write(string.format(
        "sf v=%s n_grp=%d kept=%d dshort=%d dsurp=%d safe=%s base_act=%d r_act=%s ratio=%s st=%s base_tot=%.4g sup=%s seed=%s\n",
        tag, #gorder, kept_any, drop_short, drop_surp,
...
local Pa, Ca = fixpoints(false)
local Pb, Cb = fixpoints(true)
evaluate("a", Pa, Ca)
evaluate("b", Pb, Cb)

if VERBOSE then
    io.write("  group                                        Pa Ca Pb Cb\n")
    for _, b in ipairs(gorder) do
        local g = groups[b]
        local pa, ca, pb, cb = true, true, true, true
        for m in pairs(g.vars) do
            if not Pa[m] then pa = false end
            if not Ca[m] then ca = false end
            if not Pb[m] then pb = false end
            if not Cb[m] then cb = false end
        end
        io.write(("  %-44s %s  %s  %s  %s\n"):format(b,
            pa and "y" or ".", ca and "y" or ".", pb and "y" or ".", cb and "y" or "."))
    end
end
