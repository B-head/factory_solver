...
-- (A) info-gathering: guarded greedy import-tilt vs strict reference, anchors +
-- corpus sample, REAL targets. Refined after the first run's two failure modes:
--   * base now = strict fabricate: producible import = HI (~inf, so Vp~0 like the
--     reference), but non-producible makeup = LO (legitimate, allowed).
--   * anti-collapse guard: a cheapening is accepted only if it lowers total AND
--     keeps recipes >= 0.5*base AND keeps the target met.
--   * dust floor 1e-3 on the verdict (the reference carries ~1e-5 staged-budget
--     dust; without a floor that reads as fake 1e4x improvements).
-- Questions: where does selective import GENUINELY help (lower real violation,
-- factory preserved), where is it already clean, where does it resist?
--
--   luajit tests/research/probe_greedy_vs_ref_corpus.lua [HI] [DUMP] [LO] [file ...]
...
if #FILES == 0 then
    ...
    local sample = {
        ...
        "seed_126_both_recipe_vin_sin_p1_noq_trecipe_con_h12.lua",
        "seed_139_cycle_recipe_vin_sin_p1_noq_trecipe_con_h24.lua",
        "seed_2_both_recipe_vin_sin_p1_noq_trecipe_con_h12.lua",
        "seed_42_cycle_recipe_vin_sin_p1_noq_trecipe_con_h24.lua",
...
local function run_greedy(prob)
    ...
    local function build(cheap)
        ...
        for _, p in pairs(problem.primals) do
            if p.kind == "shortage_source" then
                if p.material and cheap[strip_temp(p.material)] then p.cost = LO
                elseif producible[p.material] then p.cost = HI       -- producible: fabricate (Vp~0)
                else p.cost = LO end                                  -- non-producible makeup: allowed
    ...
    local function evaluate(cheap)
        local problem = build(cheap)
        local x = solve(problem, prob.meta)
        local s = summarize(problem, x); s.x = x
        return s
    end
    local function candidate_Q(d, x, thr, cheap)
        local consumed = flows(x)
        local bestQ, bestC = nil, 0
        for _, line in ipairs(lines) do
            local xr = x[rkey(line)] or 0
            if xr > thr then
                local makes = false
                for _, pr in ipairs(line.products or {}) do if tn.typed_name_to_variable_name(pr) == d.m then makes = true end end
                if makes then for _, pr in ipairs(line.products or {}) do
                    local m = tn.typed_name_to_variable_name(pr)
                    if m ~= d.m and (consumed[m] or 0) > bestC and not cheap[strip_temp(m)] then bestQ = m; bestC = consumed[m] or 0 end
                end end
            end
        end
        return bestQ
    ...
    local cheap, names, n_rej = {}, {}, 0
    local bsol = evaluate(cheap)
    local cur = bsol
    for _ = 1, 8 do
        local accepted
        for _, d in ipairs(cur.dumps) do
            local Q = candidate_Q(d, cur.x, cur.thr, cheap)
            if Q then
                local trial = {}; for k in pairs(cheap) do trial[k] = true end; trial[strip_temp(Q)] = true
                local res = evaluate(trial)
                local ok = res.total < cur.total - 1e-6 and res.nrec >= 0.5 * bsol.nrec and res.T <= DUST
                if ok then accepted = { Q = Q, res = res }; break else n_rej = n_rej + 1 end
            end
        end
        if not accepted then break end
        cheap[strip_temp(accepted.Q)] = true; names[#names + 1] = strip_temp(accepted.Q); cur = accepted.res
    end
    cur.base_total = bsol.total; cur.base_nrec = bsol.nrec; cur.base_Vp = bsol.Vp; cur.cheap = names; cur.n_rej = n_rej
    return cur
end
...
io.write(string.format("======= GUARDED GREEDY (refined) vs STRICT REFERENCE  HI=%g DUMP=%g LO=%g =======\n", HI, DUMP, LO))
io.write(string.format("%-50s | ref(Vp/Vc tot rec) | base(Vp tot rec) | greedy(Vp/Vc tot rec) cheap | verdict\n", "case"))
...
for _, f in ipairs(FILES) do
    local ok, err = pcall(function()
        ...
        if r.state ~= "finished" then io.write(string.format("%-50s | ref state=%s -- skip\n", f.name:gsub("%.lua$", ""), r.state)); return end
        r.total = r.Vp + r.Vf + r.Vc
        local rrec = ref_recipes(r)
        local g = run_greedy(prob)
        ...
        io.write(string.format("%-50s | %6.4g/%-8.6g %-6.5g r%-2d | %6.4g %-8.6g r%-2d | %6.4g/%-8.6g %-8.6g r%-2d {%s} | %s\n",
            (f.tag == "ANCHOR" and "* " or "  ") .. f.name:gsub("%.lua$", ""):gsub("_p1_noq", ""),
            r.Vp, r.Vc, r.total, rrec,
            g.base_Vp, g.base_total, g.base_nrec,
            g.Vp, g.Vc, g.total, g.nrec, table.concat(g.cheap, ","), verdict))
    end)
    if not ok then io.write(string.format("%-50s | ERROR %s\n", f.name, tostring(err):sub(1, 70))) end
end
