...
Invocation: `powershell -NoProfile -ExecutionPolicy Bypass -File tests/smoke_rcon.ps1` (PowerShell 5.1 works; JSONC trailing commas are stripped before parsing `settings.json` for compatibility). Exit codes: 0 = every fixture PASS, 1 = a fixture FAILed (or no response), 2 = setup error (Factorio binary not found, RCON never came up). Adding a fixture is a builder function in `manage/smoke_rcon.lua` plus its name in the launcher's `$Fixtures` — no extra boot.
