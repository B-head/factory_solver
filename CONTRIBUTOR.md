...
For the layers the headless suite can't reach — [manage/pre_solve.lua](manage/pre_solve.lua)'s machine-speed / module / quality folding, [manage/virtual.lua](manage/virtual.lua)'s virtual-recipe generation, [manage/save.lua](manage/save.lua)'s migration paths, and the GUI lifecycle — a single end-to-end smoke test boots a real Factorio with the mod loaded, constructs a minimal `Solution`, and waits for the solver pump to converge.
...
- **Stale lock file.** Every run ends with the launcher killing the Factorio process, which leaves `%APPDATA%/Factorio/.lock` behind. The launcher clears it on startup, but only when `Get-Process factorio` returns nothing — the lock is never yanked from a live instance.
- **Wall-clock cost.** Factorio bootstrap (data stage + atlas mipmaps + game init) dominates; the solver itself converges in a handful of ticks once the load completes. This is not an inner-loop check (the headless suite is); the smoke is a pre-release / pre-merge sanity gate.

Invocation: `powershell -NoProfile -ExecutionPolicy Bypass -File tests/smoke.ps1` (PowerShell 5.1 works; JSONC trailing commas are stripped before parsing `settings.json` for compatibility). Exit codes: 0 = PASS, 1 = FAIL or no marker, 2 = setup error (Factorio binary not found, log file missing).
